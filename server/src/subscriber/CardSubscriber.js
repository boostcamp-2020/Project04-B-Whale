...
export class CardSubscriber {
    ...
    async afterInsert(event) {
        ...
        await activityService.createActivity(
            ...
            `${user.name}님이 ${event.entity.title} 카드를 ${namespace?.get(
                'listTitle',
            )} 리스트에 생성하였습니다.`,
        );
    }
    ...
    async afterUpdate(event) {
        ...
        await activityService.createActivity(
            ...
            `${user.name}님이 ${event.databaseEntity.title} 카드를 ${prevList.title} 리스트에서 ${currentList.title} 리스트로 이동하였습니다.`,
        );
    }
    ...
    async afterRemove(event) {
        ...
        await activityService.createActivity(
            namespace.get('boardId'),
            `${user.name}님이 ${event.databaseEntity.title} 카드를 삭제하였습니다.`,
        );
    }
}
